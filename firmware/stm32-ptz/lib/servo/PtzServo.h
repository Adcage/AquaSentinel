   ...
   public:
    ...
    private:
     HardwareTimer* pwmTimer = nullptr;
      ...
      uint8_t panTarget = 90;
      uint8_t tiltTarget = 90;
      ...
      int16_t panVelocityUs = 0;
      int16_t tiltVelocityUs = 0;
      uint32_t lastSmoothUpdateMs = 0;
     ...
     uint16_t tiltCenterUs = 1500;
      ...
      uint16_t panTargetUs = 1500;
      uint16_t tiltTargetUs = 1500;
      CalibStorage calibStorage;
       ...
       static uint8_t pulseUsToAngle(uint16_t minUs, uint16_t centerUs, uint16_t maxUs, uint16_t pulseUs);
