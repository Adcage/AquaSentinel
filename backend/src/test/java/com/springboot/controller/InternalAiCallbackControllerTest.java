...
class InternalAiCallbackControllerTest {
    ...
    @Mock private HmacSignatureVerifier hmacSignatureVerifier;
    ...
    @Mock private AlertPushService alertPushService;

    @Mock private AlertDispatchRoutingService alertDispatchRoutingService;
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(
                controller, "alertDispatchRoutingService", alertDispatchRoutingService);
        ReflectionTestUtils.setField(controller, "lifeguardService", lifeguardService);
        ReflectionTestUtils.setField(
                controller, "applicationEventPublisher", applicationEventPublisher);
    }
    ...
    void receiveEventShouldPushToAllOnDutyVenueLifeguards() {
        ...
        when(hmacSignatureVerifier.verify(anyString(), anyString(), anyString(), anyString()))
                .thenReturn(true);
        ...
        when(lifeguardService.list(any(QueryWrapper.class)))
                .thenReturn(List.of(venueLifeguard1, venueLifeguard2));
