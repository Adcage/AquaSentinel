...
public class StatsSnapshotServiceImpl extends ServiceImpl<StatsSnapshotMapper, StatsSnapshot>
    ...
    @Override
    public QueryWrapper<StatsSnapshot> getQueryWrapper(
            StatsSnapshotQueryRequest statsSnapshotQueryRequest) {
        if (statsSnapshotQueryRequest == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "请求参数为空");
        }
        QueryWrapper<StatsSnapshot> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(
                statsSnapshotQueryRequest.getId() != null, "id", statsSnapshotQueryRequest.getId());
        queryWrapper.eq(
                StringUtils.isNotBlank(statsSnapshotQueryRequest.getGranularity()),
                "granularity",
                statsSnapshotQueryRequest.getGranularity());
        queryWrapper.eq(
                statsSnapshotQueryRequest.getSnapshotDate() != null,
                "snapshot_date",
                statsSnapshotQueryRequest.getSnapshotDate());
        queryWrapper.eq(
                statsSnapshotQueryRequest.getSnapshotHour() != null,
                "snapshot_hour",
                statsSnapshotQueryRequest.getSnapshotHour());
        queryWrapper.eq(
                statsSnapshotQueryRequest.getVenueId() != null,
                "venue_id",
                statsSnapshotQueryRequest.getVenueId());
        queryWrapper.eq(
                StringUtils.isNotBlank(statsSnapshotQueryRequest.getMetricType()),
                "metric_type",
                statsSnapshotQueryRequest.getMetricType());
        queryWrapper.eq(
                StringUtils.isNotBlank(statsSnapshotQueryRequest.getMetricKey()),
                "metric_key",
    ...
    public List<StatsSnapshotVO> getStatsSnapshotVO(List<StatsSnapshot> statsSnapshotList) {
        ...
        return statsSnapshotList.stream()
                .map(this::getStatsSnapshotVO)
