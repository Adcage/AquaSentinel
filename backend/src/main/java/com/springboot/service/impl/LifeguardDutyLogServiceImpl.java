...
*/
@Service
public class LifeguardDutyLogServiceImpl extends ServiceImpl<LifeguardDutyLogMapper, LifeguardDutyLog>
    ...
    public void validLifeguardDutyLog(LifeguardDutyLog lifeguardDutyLog, boolean add) {
        ...
        if (StringUtils.isNotBlank(lifeguardDutyLog.getAction_type()) && lifeguardDutyLog.getAction_type().length() > 32) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "动作类型过长");
        }
    }

    @Override
    public QueryWrapper<LifeguardDutyLog> getQueryWrapper(LifeguardDutyLogQueryRequest lifeguardDutyLogQueryRequest) {
        if (lifeguardDutyLogQueryRequest == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "请求参数为空");
        }
        QueryWrapper<LifeguardDutyLog> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(lifeguardDutyLogQueryRequest.getId() != null, "id", lifeguardDutyLogQueryRequest.getId());
        queryWrapper.eq(lifeguardDutyLogQueryRequest.getLifeguardId() != null,
                "lifeguard_id", lifeguardDutyLogQueryRequest.getLifeguardId());
        queryWrapper.eq(StringUtils.isNotBlank(lifeguardDutyLogQueryRequest.getActionType()),
                "action_type", lifeguardDutyLogQueryRequest.getActionType());
        queryWrapper.eq(lifeguardDutyLogQueryRequest.getApprovedBy() != null,
                "approved_by", lifeguardDutyLogQueryRequest.getApprovedBy());
        queryWrapper.eq(lifeguardDutyLogQueryRequest.getPlannedReturnAt() != null,
                "planned_return_at", lifeguardDutyLogQueryRequest.getPlannedReturnAt());
        queryWrapper.eq(lifeguardDutyLogQueryRequest.getActualReturnAt() != null,
                "actual_return_at", lifeguardDutyLogQueryRequest.getActualReturnAt());
    ...
    @Override
    public List<LifeguardDutyLogVO> getLifeguardDutyLogVO(List<LifeguardDutyLog> lifeguardDutyLogList) {
        if (CollUtil.isEmpty(lifeguardDutyLogList)) {
            return new ArrayList<>();
        }
        return lifeguardDutyLogList.stream().map(this::getLifeguardDutyLogVO).collect(Collectors.toList());
    }
