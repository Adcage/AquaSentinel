...
public class SystemAuditLogServiceImpl extends ServiceImpl<SystemAuditLogMapper, SystemAuditLog>
    ...
    @Override
    public QueryWrapper<SystemAuditLog> getQueryWrapper(SystemAuditLogQueryRequest systemAuditLogQueryRequest) {
        if (systemAuditLogQueryRequest == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "请求参数为空");
        }
        QueryWrapper<SystemAuditLog> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(systemAuditLogQueryRequest.getId() != null, "id", systemAuditLogQueryRequest.getId());
        queryWrapper.eq(StringUtils.isNotBlank(systemAuditLogQueryRequest.getTraceId()), "trace_id",
                systemAuditLogQueryRequest.getTraceId());
        queryWrapper.eq(StringUtils.isNotBlank(systemAuditLogQueryRequest.getLogCategory()), "log_category",
                systemAuditLogQueryRequest.getLogCategory());
        queryWrapper.eq(systemAuditLogQueryRequest.getOperatorId() != null, "operator_id", systemAuditLogQueryRequest.getOperatorId());
        queryWrapper.like(StringUtils.isNotBlank(systemAuditLogQueryRequest.getOperatorName()), "operator_name",
                systemAuditLogQueryRequest.getOperatorName());
        queryWrapper.like(StringUtils.isNotBlank(systemAuditLogQueryRequest.getRequestUri()), "request_uri",
                systemAuditLogQueryRequest.getRequestUri());
        queryWrapper.eq(systemAuditLogQueryRequest.getResponseCode() != null, "response_code", systemAuditLogQueryRequest.getResponseCode());
        queryWrapper.ge(systemAuditLogQueryRequest.getStartCreatedAt() != null, "created_at", systemAuditLogQueryRequest.getStartCreatedAt());
        queryWrapper.le(systemAuditLogQueryRequest.getEndCreatedAt() != null, "created_at", systemAuditLogQueryRequest.getEndCreatedAt());
    ...
    public List<SystemAuditLogVO> getSystemAuditLogVO(List<SystemAuditLog> systemAuditLogList) {
        ...
        return systemAuditLogList.stream().map(this::getSystemAuditLogVO).collect(Collectors.toList());
    }
}
