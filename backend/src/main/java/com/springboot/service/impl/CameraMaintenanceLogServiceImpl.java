...
*/
@Service
public class CameraMaintenanceLogServiceImpl extends ServiceImpl<CameraMaintenanceLogMapper, CameraMaintenanceLog>
    ...
    public void validCameraMaintenanceLog(CameraMaintenanceLog cameraMaintenanceLog, boolean add) {
        ...
        if (cameraMaintenanceLog.getMaintained_at() != null && cameraMaintenanceLog.getNext_maintenance_at() != null
                && cameraMaintenanceLog.getNext_maintenance_at().before(cameraMaintenanceLog.getMaintained_at())) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "下次维护时间不能早于维护时间");
        }
        if (cameraMaintenanceLog.getCamera_id() != null && StringUtils.isNotBlank(cameraMaintenanceLog.getMaintenance_type())
            ...
            queryWrapper.ne(cameraMaintenanceLog.getId() != null, "id", cameraMaintenanceLog.getId());
    ...
    @Override
    public QueryWrapper<CameraMaintenanceLog> getQueryWrapper(CameraMaintenanceLogQueryRequest cameraMaintenanceLogQueryRequest) {
        if (cameraMaintenanceLogQueryRequest == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "请求参数为空");
        }
        QueryWrapper<CameraMaintenanceLog> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(cameraMaintenanceLogQueryRequest.getId() != null, "id", cameraMaintenanceLogQueryRequest.getId());
        queryWrapper.eq(cameraMaintenanceLogQueryRequest.getCameraId() != null, "camera_id",
                cameraMaintenanceLogQueryRequest.getCameraId());
        queryWrapper.eq(StringUtils.isNotBlank(cameraMaintenanceLogQueryRequest.getMaintenanceType()), "maintenance_type",
                cameraMaintenanceLogQueryRequest.getMaintenanceType());
        queryWrapper.like(StringUtils.isNotBlank(cameraMaintenanceLogQueryRequest.getMaintenanceContent()), "maintenance_content",
                cameraMaintenanceLogQueryRequest.getMaintenanceContent());
        queryWrapper.like(StringUtils.isNotBlank(cameraMaintenanceLogQueryRequest.getMaintainedBy()), "maintained_by",
                cameraMaintenanceLogQueryRequest.getMaintainedBy());
        queryWrapper.ge(cameraMaintenanceLogQueryRequest.getStartMaintainedAt() != null, "maintained_at",
                cameraMaintenanceLogQueryRequest.getStartMaintainedAt());
        queryWrapper.le(cameraMaintenanceLogQueryRequest.getEndMaintainedAt() != null, "maintained_at",
    ...
    @Override
    public CameraMaintenanceLogVO getCameraMaintenanceLogVO(CameraMaintenanceLog cameraMaintenanceLog) {
        if (cameraMaintenanceLog == null) {
            return null;
        }
    ...
    @Override
    public List<CameraMaintenanceLogVO> getCameraMaintenanceLogVO(List<CameraMaintenanceLog> cameraMaintenanceLogList) {
        if (CollUtil.isEmpty(cameraMaintenanceLogList)) {
            return new ArrayList<>();
        }
        return cameraMaintenanceLogList.stream().map(this::getCameraMaintenanceLogVO).collect(Collectors.toList());
    }
}
